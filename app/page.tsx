...
export default function Home() {
  ...
  const visibleEntries = useMemo(
    () => filterEntries(entries, month, personFilter, projectFilter),
    [entries, month, personFilter, projectFilter]
  );
  ...
  async function loadEntries() {
    try {
      ...
      setStatus(data.configured === false ? "请配置 Upstash 环境变量后开始记录" : "Upstash 已连接");
      setStatusKind(data.configured === false ? "error" : "ready");
    } catch (error) {
  ...
  function exportSummaryCsv() {
    const rows: Array<Array<string | number>> = [
      ...
      ["记录数", summary.entryCount],
      [],
      ["按人员"],
      ["人员", "工时", "记录数"],
      ...summary.people.map((item) => [item.name, item.hours, item.count]),
      [],
      ["按项目"],
      ["项目", "工时", "记录数"],
      ...summary.projects.map((item) => [item.name, item.hours, item.count]),
      [],
      ["明细"],
      ["日期", "人员", "项目", "工时", "工作备注"],
      ...visibleEntries.map((entry) => [entry.date, entry.person, entry.project, entry.hours, entry.notes])
    ];

    const safePerson = personFilter === "全部" ? "all-people" : personFilter;
    const safeProject = projectFilter === "全部" ? "all-projects" : projectFilter.replace(/\s+/g, "-");
    downloadCsv(`monthly-summary-${month}-${safePerson}-${safeProject}.csv`, rows);
  }
        ...
        {!user ? (
          ...
          <>
            <div className={`tabbar ${user.isAdmin ? "admin-tabs" : "account-tabs"}`} role="tablist" aria-label="工作记录视图">
              <button aria-controls="entry-panel" aria-selected={activeTab === "entry"} className="tab-button" id="entry-tab" role="tab" type="button" onClick={() => setActiveTab("entry")}>
                报时
              </button>
              <button aria-controls="summary-panel" aria-selected={activeTab === "summary"} className="tab-button" id="summary-tab" role="tab" type="button" onClick={() => setActiveTab("summary")}>
                月度汇总
              </button>
              <button aria-controls="settings-panel" aria-selected={activeTab === "settings"} className="tab-button" id="settings-tab" role="tab" type="button" onClick={() => setActiveTab("settings")}>
                设置
              </button>
              {user.isAdmin ? (
                <button aria-controls="users-panel" aria-selected={activeTab === "users"} className="tab-button" id="users-tab" role="tab" type="button" onClick={() => setActiveTab("users")}>
                  用户
                </button>
              ) : null}
            </div>
            ...
            <div className="workspace">
                ...
                <section aria-labelledby="entry-tab" className="panel tab-panel" id="entry-panel" role="tabpanel">
                  ...
                  <form className="entry-form" onSubmit={submitEntry}>
                    <div className="two-col">
                      ...
                      <label className="field">
                        <span>工时</span>
                        <input required min="0.25" max="24" step="0.25" type="number" value={form.hours} onChange={(event) => setForm({ ...form, hours: Number(event.target.value) })} />
                      </label>
                    </div>
                    <label className="field">
                      <span>人员</span>
                    ...
                    <label className="field">
                      <span>工作备注</span>
                      <textarea rows={3} placeholder="记录本次工作的主要内容或产出" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} />
                    </label>
              ...
              {activeTab === "summary" ? (
                <section aria-labelledby="summary-tab" className="panel tab-panel" id="summary-panel" role="tabpanel">
                  ...
                  <div className="filters summary-filters">
                    <label className="field">
                      <span>月份</span>
                  ...
                  <div className="summary-actions">
                    <button className="secondary-btn export-btn" type="button" onClick={exportSummaryCsv}>
                      导出汇总表格
                    </button>
                  </div>
                  <div className="summary-grid">
                    <div className="metric"><span className="label">总工时</span><span className="value">{summary.totalHours}</span></div>
                    <div className="metric"><span className="label">记录数</span><span className="value">{summary.entryCount}</span></div>
                  </div>
                  <SummaryBars title="按人员" buckets={summary.people} />
                  <SummaryBars title="按项目" buckets={summary.projects} />
                  <div className="summary-section"><h3>明细</h3></div>
                          ...
                          <article className="entry-card" key={entry.id}>
                            {isEditing ? (
                              <form className="inline-edit-form" onSubmit={submitEditEntry}>
                                <div className="two-col">
                                  <label className="field"><span>日期</span><input required type="date" value={editForm.date} onChange={(event) => setEditForm({ ...editForm, date: event.target.value })} /></label>
                                  <label className="field"><span>工时</span><input required min="0.25" max="24" step="0.25" type="number" value={editForm.hours} onChange={(event) => setEditForm({ ...editForm, hours: Number(event.target.value) })} /></label>
                                </div>
                                <label className="field"><span>人员</span><select required value={editForm.person} onChange={(event) => setEditForm({ ...editForm, person: event.target.value })}>{peopleOptions.map((person) => <option key={person} value={person}>{person}</option>)}</select></label>
                                <label className="field"><span>项目</span><select required value={editForm.project} onChange={(event) => setEditForm({ ...editForm, project: event.target.value })}>{projectOptions.map((project) => <option key={project} value={project}>{project}</option>)}</select></label>
                                <label className="field"><span>工作备注</span><textarea rows={3} value={editForm.notes} onChange={(event) => setEditForm({ ...editForm, notes: event.target.value })} /></label>
                                <div className="entry-actions">
                                  <button className="primary-btn compact-btn" disabled={isUpdatingEntry} type="submit">{isUpdatingEntry ? "保存中" : "保存"}</button>
                                  <button className="secondary-btn compact-btn" type="button" onClick={cancelEditEntry}>取消</button>
                                </div>
                              </form>
                            ) : (
                              <>
                                <div className="entry-title"><span>{entry.project}</span><span>{entry.hours}h</span></div>
                                <div className="entry-meta"><span className="tag">{entry.date}</span><span className="tag">{entry.person}</span></div>
                                {entry.notes ? <p className="entry-note">{entry.notes}</p> : null}
                                <div className="entry-actions">
                                  <button className="secondary-btn compact-btn" type="button" onClick={() => startEditEntry(entry)}>编辑</button>
                                  <button className="danger-btn compact-btn" disabled={deletingEntryId === entry.id} type="button" onClick={() => deleteWorkEntry(entry)}>{deletingEntryId === entry.id ? "删除中" : "删除"}</button>
                                </div>
                              </>
                            )}
                          </article>
              ...
              {activeTab === "settings" ? (
                <section aria-labelledby="settings-tab" className="panel tab-panel" id="settings-panel" role="tabpanel">
                  <div className="panel-header"><h2 id="settings-title">账号设置</h2><p>修改用户名或密码时，需要输入当前密码确认。</p></div>
                  <form className="entry-form" onSubmit={submitSettings}>
                    <label className="field"><span>用户名</span><input required autoComplete="username" minLength={2} value={settingsForm.username} onChange={(event) => setSettingsForm({ ...settingsForm, username: event.target.value })} /></label>
                    <label className="field"><span>新密码</span><input autoComplete="new-password" minLength={6} placeholder="不修改密码可留空" type="password" value={settingsForm.newPassword} onChange={(event) => setSettingsForm({ ...settingsForm, newPassword: event.target.value })} /></label>
                    <label className="field"><span>当前密码</span><input required autoComplete="current-password" minLength={6} placeholder="请输入当前密码确认修改" type="password" value={settingsForm.currentPassword} onChange={(event) => setSettingsForm({ ...settingsForm, currentPassword: event.target.value })} /></label>
                    <button className="primary-btn" disabled={isUpdatingProfile} type="submit">{isUpdatingProfile ? "更新中" : "保存设置"}</button>
                  </form>
                </section>
              ) : null}
              ...
              {activeTab === "users" && user.isAdmin ? (
                <section aria-labelledby="users-tab" className="panel tab-panel" id="users-panel" role="tabpanel">
                  <div className="panel-header"><h2 id="users-title">新增用户</h2><p>只有管理员账号可以创建新用户。新用户创建后可直接登录报时。</p></div>
                  <form className="entry-form" onSubmit={createRegularUser}>
                    <label className="field"><span>用户名</span><input required autoComplete="off" minLength={2} placeholder="请输入新用户名" value={newUserForm.username} onChange={(event) => setNewUserForm({ ...newUserForm, username: event.target.value })} /></label>
                    <label className="field"><span>初始密码</span><input required autoComplete="new-password" minLength={6} placeholder="至少 6 个字符" type="password" value={newUserForm.password} onChange={(event) => setNewUserForm({ ...newUserForm, password: event.target.value })} /></label>
                    <button className="primary-btn" disabled={isCreatingUser} type="submit">{isCreatingUser ? "创建中" : "创建用户"}</button>
                  </form>
                </section>
              ) : null}
            </div>
